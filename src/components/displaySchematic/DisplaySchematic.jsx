...
function DisplaySchematic({ schematic, index, popSchematic }) {
  ...
  async function downloadSchematic(event) {
    ...
    const response = await fetch(`https://mc-schematic-manager-server.adaptable.app/get-schematic-file/${id}`);
    if (response.ok) {
  ...
  async function getSchematicString(event) {
    try {
      const id = event.target.name;
      const response = await fetch(`https://mc-schematic-manager-server.adaptable.app/get-schematic-fawe-string/${id}`);
  ...
  async function removeSchematic(event) {
    const id = event.target.name;
    popSchematic(event);
    const removeStatus = await fetch(`https://mc-schematic-manager-server.adaptable.app/remove-schematic/${id}`)
      .then((response) => {
